...
if __name__ == "__main__":
    ...
    file_path = f"{AUDIO}/{AUDIO}/htdemucs/{AUDIO}.mp3"
    ...
    # --- Save Everything ---
    np.savez_compressed(f"{AUDIO}/{AUDIO}_analysis.npz", 
                        hpcp=frame_hpcps, 
                        beats=beat_map, 
    ...
    print(f"For: {AUDIO}")
    print(f"Done! Saved {num_frames} frames ({duration_sec:.2f}s of frame timeline).")
    print(f"Detected BPM: {bpm:.2f} | method: {selected_method} | confidence: {confidence:.2f}")