...
def unpack_lyrics_1(parent):
    ...
    for broad_chunk in json_data: #TODO: Update to show total text, broad chunk and then highlight the current word.
        for word in broad_chunk["words"]:
            ...
            end_1 = word["end"]
            word_1 = word["word"]
            lyrics1.append((start_1, end_1, word_1))


def update_lyric_1(ms):
    global i1
    elapsed = ms / 1000
    try:
        start = lyrics1[i1][0]
        ...
        if elapsed > 1:
            if start <= elapsed < end:
                lyric_text1 = font.render(f"{word}", True, (255, 255, 255))
                screen.blit(lyric_text1, (50, 100))
