...
class Alignment:
    ...
    def find_word_index(self, seg_idx: int, t: float) -> Optional[int]:
        ...
        # Debug: log gap
        if s.words:
            first_word_start = s.words[0].start
            last_word_end = s.words[-1].end
            if t < first_word_start or t > last_word_end:
                print(f"[DEBUG] Segment {seg_idx} t={t:.3f} outside word range [{first_word_start:.3f}, {last_word_end:.3f}]")
        return None
...
class MainWindow(QMainWindow):
    ...
    def _on_position(self, pos_ms: int):
        ...
        seg = self.alignment.segments[seg_idx]
        word_idx = self.alignment.find_word_index(seg_idx, t)
        
        # Debug logging
        if word_idx is None and seg.words:
            first_start = seg.words[0].start
            last_end = seg.words[-1].end
            print(f"[DEBUG] No word found in segment {seg_idx} at t={t:.3f}. Segment time: [{seg.start:.3f}, {seg.end:.3f}]. Words: [{first_start:.3f}, {last_end:.3f}]")
